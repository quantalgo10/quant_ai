...
def get_data(symbol, period='1d', interval='2m'): # Change Interval Here 1,2,5,15
...
def calculate_rsi(prices, period=14):
    ...
    rs = gain / loss
    rsi = 100 - (100 / (1 + rs))
    return rsi.fillna(50)  # Default RSI to neutral (50) if NaN
...
def display_portfolio_dashboard(df, symbol):
    ...
    if not trade_df.empty:
        ...
        with tab3:
            # Trade Duration Analysis - Fixed version
            closed_trades['Duration'] = closed_trades.apply(
                lambda row: (row['Exit Time'] - row['Entry Time']).total_seconds() / 60 
                if pd.notnull(row['Exit Time']) and pd.notnull(row['Entry Time']) 
                else 0, 
                axis=1
            )
...
#         st.session_state.last_refresh = current_time
#         st.experimental_rerun()  # Ensure this triggers a clean rerun
